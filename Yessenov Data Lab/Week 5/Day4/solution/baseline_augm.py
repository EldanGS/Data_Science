...
import numpy as np
from simple_augmentation import augment_data
...
FLAGS_NET['results_name_suffix'] = '_augmented'
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        for i in tqdm(range(20000)):
            ...
            batch = (np.reshape(batch[0], newshape=(-1, 28, 28, 1)).astype(np.float32), batch[1])
            batch = augment_data(batch[0], batch[1])
            ...
            train_step.run(feed_dict={x: np.reshape(batch[0], newshape=(-1, 784)), y_: batch[1], keep_prob: 0.5})
